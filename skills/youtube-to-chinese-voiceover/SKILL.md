...
description: Use when需要把单个 YouTube 或本地视频素材做成低成本、可审计的中文旁白版，并把转写、中文稿、配音、对齐、字幕和交付物完整沉淀到 reports/ 中。
---
...
这个 skill 面向**单视频、个人可运行、强调可复现**的中文旁白工作流。目标不是声纹克隆，也不是口型同步；目标是把一个视频真实跑通为“中文旁白版”，并把证据、命令、问题与交付物完整留在 `reports/` 中。
...
5. **生成真实中文 TTS**
   ...
   - 建议使用真实普通话音色，例如 `zh-CN-XiaoxiaoNeural`。
   - 沉默文件、占位文件、空白音轨都不算完成。
